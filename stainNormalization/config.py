...
def get_config(FLAGS, is_train):
  ...
  if is_train:
    config.im_size =  412# The width and height should be equal. Upper bound of the input image size is limited to the GPU memory.
    config.lr = 1e-4
    ...
    config.batch_size = 1
    config.im_size = 452# The width and height should be equal. Upper bound of the input image size is limited to the GPU memory.
